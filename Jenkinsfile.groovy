...
node(){
  try{
    ...
    stage('SERVICE - Git checkout'){
      git branch: branchName, url: "http://gitlab.esgi.lab/dev/fycapp.git"
    }
    ...
    def version = sh returnStdout: true, script: "cat pom.xml | grep -A1 '<artifactId>projet-fyc' | tail -1 |perl -nle 'm{.*<version>(.*)</version>.*};print \$1' | tr -d '\n'"
    ...
    def imageName='192.168.1.123:5000/fyc:latest'

    stage('DOCKER - Build/Push registry'){
      docker.withRegistry('http://192.168.1.123:5000', 'myregistry_login') {
    ...
    stage('DOCKER - check registry'){
      ...
      sh 'curl -sk --user $USERNAME:$PASSWORD https://192.168.1.123:5000/v2/fyc/tags/list'
      }
    }
